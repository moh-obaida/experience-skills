...
This catalog was the v0.1 research summary. From v0.2.0, real-world research is organized as:
...
- **Interpretation:** `shared/precedent/` — 15 concept modules (intentional space vs dead space,
  ...
  discovery, product-first presentation, justified trends). Each entry states what works, why, when it
  is right, what does not transfer, and when copying it fails.
- **Design library:** `shared/design-intelligence/` cites observation IDs in directions and
  compositions.

Rules carried over from v0.1: study why something works, never copy surfaces; no screenshots or
third-party assets are stored; commentary is original.
